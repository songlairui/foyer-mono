...
export function RepoDetailModal({ repo, agentOnline, onOpen, onClose }: RepoDetailModalProps) {
  ...
  return (
    <div
      className={cn(
        ...
        entering ? "bg-black/0" : "bg-black/70 backdrop-blur-sm",
      )}
    ...
    >
      ...
      <button
        onClick={onClose}
        className="absolute top-6 right-6 z-50 flex h-8 w-8 items-center justify-center rounded-full bg-white/10 text-white/70 backdrop-blur transition-colors hover:bg-white/20 hover:text-white"
      >
        <X className="h-4 w-4" />
      </button>
      ...
      <div
        className={cn(
          ...
          entering ? "scale-50 opacity-0" : "scale-100 opacity-100",
        )}
        style={{ perspective: "1200px" }}
      >
        <div
          className={cn(
            "relative w-[380px] min-h-[320px] transition-transform duration-700 ease-[cubic-bezier(0.4,0,0.2,1)]",
          )}
          style={{
        ...
        >
          {/* ── 正面：仓库详情 ── */}
          <div
            className="absolute inset-0 rounded-2xl border border-white/15 bg-card/95 p-6 flex flex-col gap-4 shadow-2xl"
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className="flex items-center gap-1 shrink-0 rounded-md border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-[11px] text-amber-400 hover:bg-amber-500/20 transition-colors cursor-pointer"
                    >
          ...
          {/* ── 背面：Worktree 列表 ── */}
          <div
            className="absolute inset-0 rounded-2xl border border-amber-500/20 bg-card/95 p-6 flex flex-col gap-4 shadow-2xl"
            style={{
