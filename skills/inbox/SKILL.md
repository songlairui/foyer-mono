...
description: |
  ...
  显式触发：「inbox」「inbox xxx」「丢一个想法」「梳理一下 inbox」「_inbox 那个」等。
  自主触发：用户在对话中表达新想法、功能需求、缺陷报告、设计疑问、或任何值得留存的事项时，
  即使没有显式提及 inbox，也应主动调用 `inbox <内容>` 进行捕获。
  skill 内部会自动分级：碎碎念仅记录、探索型生成设计稿并等待确认、明确执行型全自动实施。
...
除了用户显式说「inbox」之外，以下场景应**主动调用**本 skill 捕获事项：

- 用户表达新想法、灵感、突然想到的点子（无论多模糊）
- 用户提到功能需求或改进建议（「能不能加…」「最好支持…」）
- 用户报告缺陷或异常行为（「这个不对」「崩了」「报错」）
- 用户提出设计疑问或技术选型问题（「用哪种方案好」「怎么设计更合理」）
- 用户做出值得记录的决定（「决定了，用…」「就按这个方向」）
- 用户表达需要稍后处理的事项（「回头记得…」「改天弄一下…」）

**不需要触发的情况**：

- 用户给出直接执行指令（「打开 X 文件」「执行 Y 命令」「帮我查 Z」）
- 用户追问当前任务细节（纯信息获取）
- 用户对当前正在进行的实施给出修改意见（属于当前任务的延续）

调用方式：直接在回复中使用 `inbox <用户原始表达>`，skill 会完成润色、分类、分级、路由。
无需在调用前手动整理内容。
...
（改完后运行 `./sync-skills.sh` 把变更同步到 `~/.agents/skills/inbox/`。）
