...
| --- | --- | --- | --- | --- | --- | --- |
| CLI | TypeScript/Effect 程序、命令、JSON schema、exit code | 用户本机、CI、agent shell | 是 | 确定性文件/Git/GitHub/entry 操作 | 若交互协议不稳定，agent 难以可靠调用 | `entry project init` 等命令 |
...
- `references/` 放 entry 协议、失败恢复、命名规范、示例输出。
- `scripts/` 可以保留最薄的兼容脚本，但确定性主逻辑应下沉到 CLI。
- 如果 CLI 返回未知错误或参数不清楚，优先读取 `entry project init --help` 或对应子命令 help，而不是猜测实现细节。
...
- 注入动态上下文，例如当前 entry root、projects root、device name、GitHub 登录状态。
...
- 可以用 local sandbox 复用本机仓库和 `~/entry`，也可以在未来切到隔离 sandbox。
