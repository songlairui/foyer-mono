...
```bash
entry activity query --project entry-init-project --since 2026-05-01 --json
entry activity context --project entry-init-project --budget 4000 --format markdown
entry activity export --scope project:entry-init-project --target graphify-corpus
entry activity export --scope project:entry-init-project --target hyperextract-input
```
...
```bash
entry activity query --event project.created --since today --json
entry project timeline <slug> --format markdown
```
...
```bash
entry activity export --scope all-projects --target graphify-corpus --out .entry-derived/graphify/all-projects/corpus
...
```bash
entry activity export --scope project:<slug> --target hyperextract-input --out .entry-derived/hyperextract/<slug>/input.md
...
```bash
entry activity export --since-cursor graphify-all-projects --target graphify-corpus
graphify .entry-derived/graphify/all-projects/corpus --update

entry activity export --since-cursor hyperextract-entry-init-project --target markdown-fragment
...
```bash
entry search "Flue agent 交付形式" --project entry-init-project --limit 10 --json
entry get event <event-id> --format markdown
```
...
也可以把大体积派生物放到项目工作区或缓存目录，避免污染 `~/entry`：
...
原则：`~/entry/activity/events` 是事实；`derived` 是缓存。缓存可删，事实不可丢。
...
```bash
entry activity query --project <slug> --since 2026-05-01 --json
entry activity context --project <slug> --budget 6000 --format markdown
entry search "关键词" --json
entry get event <event-id> --format markdown
```
...
```bash
entry export text --scope project:<slug> --out derived/text/<slug>.md
entry export corpus --scope all-projects --out derived/graphify/all-projects/corpus
entry export hyperextract --scope project:<slug> --template entry/project_timeline --out derived/hyperextract/<slug>/input.md
```

### 派生层

```bash
entry derive graphify --scope all-projects --update
entry derive hyperextract --scope project:<slug> --template entry/project_timeline --feed
entry derive fts --rebuild
```

### Agent 入口

```bash
entry ask "这个项目下一步是什么？" --project <slug>
entry graph query "skill、flue、cli 的关系是什么？"
entry ka search <slug> "最近的决策是什么？"
```
...
1. 定义 `entry activity query/context/export` 的 CLI contract。
